<template>
  <RouterLink
    ...
    :style="isActive
      ? { background: 'linear-gradient(135deg, var(--rose-primary), var(--rose-600))' }
...
function handleMouseEnter(e) {
  if (!isActive.value) e.target.style.background = 'var(--bg-surface)'
}

function handleMouseLeave(e) {
  if (!isActive.value) e.target.style.background = 'transparent'
}
